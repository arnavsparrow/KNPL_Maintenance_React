...
function deriveTagFields(
  description: string,
): Pick<MqttTag, "displayName" | "unit" | "decimals" | "maxValue"> {
  const unitMatch = description.match(/\(([^)]+)\)\s*$/);
  const unit = unitMatch?.[1] ?? "";
  const text = description.replace(/\s*\([^)]+\)\s*$/, "");
  const displayName = text.includes(" – ")
    ? text.split(" – ").slice(1).join(" – ")
    : text;

  switch (unit) {
    case "A":
      return { displayName, unit, decimals: 1, maxValue: 80 };
    case "RPM":
      return { displayName, unit, decimals: 0, maxValue: 5000 };
    case "°C":
      return { displayName, unit, decimals: 1, maxValue: 220 };
    case "bar":
      return { displayName, unit, decimals: 1, maxValue: 25 };
    case "Nm":
      return { displayName, unit, decimals: 0, maxValue: 400 };
    case "mm/s":
      return { displayName, unit, decimals: 1, maxValue: 10 };
    case "m/min":
      return { displayName, unit, decimals: 1, maxValue: 2 };
    case "kg":
      return { displayName, unit, decimals: 1, maxValue: 200 };
    default:
      return { displayName, unit, decimals: 1, maxValue: 100 };
  }
}
...
const TagMasterPage: React.FC = () => {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof Omit<MqttTag, "id">, string>>
  >({});
  ...
  function validate(): boolean {
    ...
    const isDuplicate = tags.some(
      (t) => t.tagId === form.tagId.trim() && t.id !== editingId,
    );
    if (isDuplicate) e.tagId = "Tag ID already exists";
    const isDupTopic = tags.some(
      (t) => t.topicName === form.topicName.trim() && t.id !== editingId,
    );
  ...
  function handleSave() {
    ...
    const payload: Omit<MqttTag, "id"> = {
      ...form,
      ...deriveTagFields(description),
    ...
    if (editingId) {
      ...
      setTags((prev) => [...prev, { id: nextId(prev), ...payload }]);
    }
    setModalOpen(false);
  }
  ...
  return (
    <div className="fullpage active">
      <div className="master-header">
        <div className="master-title-wrap">
          ...
          <div className="master-sub">
            Manage MQTT tags — topic names used for real-time sensor data
            ingestion
          </div>
        </div>
          ...
          <table>
            ...
            <tbody>
              ...
              {filtered.map((tag, idx) => (
                <tr key={tag.id}>
                  <td className="text-mono" style={{ color: "#94A3B8" }}>
                    {idx + 1}
                  </td>
                  <td>
          ...
          <div className="modal-header">
            <div>
              <div className="modal-title">
                {editingId ? "✏ Edit MQTT Tag" : "+ Add MQTT Tag"}
              </div>
              <div className="modal-subtitle">
                Fill in the tag details below
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  value={form.tagId}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, tagId: e.target.value }))
                  }
                />
                {errors.tagId && (
                  <span className="form-error">{errors.tagId}</span>
                )}
              </div>
              <div className="form-group">
                <label className="form-label">
                  MQTT Topic Name <span className="required">*</span>
                </label>
                <input
                ...
                />
                {errors.topicName && (
                  <span className="form-error">{errors.topicName}</span>
                )}
                <span className="form-hint">
                  Use forward-slash path format, e.g. plant/equipment/sensor
                </span>
              </div>
              <div className="form-group">
                <label className="form-label">
      ...
      {/* Delete Confirm */}
      <div
        className={`modal-overlay${deleteConfirmId ? " open" : ""}`}
        onClick={(e) => {
          ...
          <div className="modal-body">
            <div className="escalation-alert" style={{ marginBottom: 0 }}>
              ...
              <div className="escalation-alert-text">
                ...
                <p>
                  Deleting this tag will permanently remove it from the master
                  list.
                </p>
              </div>
            </div>
